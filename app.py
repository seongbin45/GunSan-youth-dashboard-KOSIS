...
with col1:
    st.page_link("pages/1_혜택찾기.py", label="🎁 내 혜택 찾기", icon="🎁")
...
with st.expander("✅ **핵심만 간단하게 요약한 용어 사전**"):
    st.markdown("""
✅ **핵심만 간단하게 요약한 용어 사전**
* **기준 중위소득 (나의 소득 등수)**  
...
with st.expander("💗 **모두를 위해, 조금 더 쉽게 설명해 드릴게요**"):
    st.markdown("""
    ...
    if 20 <= age <= 34:
        ...
        with st.expander("📌 2026년 국가건강검진 - 청년 정신건강 확대 (중앙정부)"):
            ...
            * **안내**: 해당 연도 건강검진 대상 청년이라면 무료로 검진받을 수 있습니다.
            """)
            st.markdown("⚠️ 참고용으로만 사용하시기 바랍니다. 의학적인 자문이나 진단이 필요한 경우 전문가에게 문의하세요.")
...
**🏢 데이터 출처:** 국가통계포털(KOSIS) - KOSIS 통계정보 API  
...
try:
    ...
    with st.expander("""🔍 "전북특별자치도" 전체 원본 데이터 표 보기"""):
        if difficulty_df is not None:
