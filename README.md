...
### KOSIS API 키 (군산 청년 통계 동기화)

AI 통계 툴·`pages/6_군산시 청년 데이터` 갱신은 `gunsan_youth_data.db`를 사용합니다.  
**최신 KOSIS 수치로 DB를 다시 채우려면** `KOSIS_API_KEY`가 필요합니다.
...
# 키 붙여넣기 → .streamlit/secrets.toml 에 저장

# CLI 동기화도 쓰려면
python scripts/set_kosis_key.py --also-env

# 설정 여부 확인 (키 값은 마스킹만 표시)
...
1. `.streamlit/secrets.toml.example` 참고  
...
또는 `.env.example` → `.env` 복사 후 `KOSIS_API_KEY=` 입력  
(`finfit_youth.config`가 `.env`를 자동 로드합니다.)

Streamlit은 secrets 변경 후 **재시작**이 필요합니다.  
페이지 6의 **「KOSIS 데이터 갱신」** expander에서도 설정 상태·동기화 버튼을 확인할 수 있습니다.
